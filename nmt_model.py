...
class NMT(nn.Module):
    ...
    def encode(
        ...
        conct = torch.cat((last_hidden[0], last_hidden[1]), dim=1)

        # Apply the h_projection layer to this in order to compute init_decoder_hidden.
        # This is h_0^{dec} in the PDF. Here b = batch size, h = hidden size
        # - `init_decoder_cell`:
        init_decoder_hidden= self.h_projection(conct)
        
        # `last_cell` is a tensor shape (2, b, h). The first dimension corresponds to forwards and backwards.
        # Concatenate the forwards and backwards tensors to obtain a tensor shape (b, 2*h).
        conct_cell = torch.cat((last_cell[0], last_cell[1]), dim=1)

        # Apply the c_projection layer to this in order to compute init_decoder_cell.
        # This is c_0^{dec} in the PDF. Here b = batch size, h = hidden size

        init_decoder_cell = self.c_projection(conct_cell)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            dec_state, o_t, _ = self.step(Y_bar_t, 
                                          dec_state, 
                                          enc_hiddens, 
                                          enc_hiddens_proj, 
                                          enc_masks)
